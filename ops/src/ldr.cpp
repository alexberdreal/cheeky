...
namespace cheeky::ops {
    bool LdrImm::process(uint32_t bits, State &state) {
        ...
        auto str_pre_post_idx = [&state, bits = bits](auto src, auto& rn, auto& rt, auto simm9) {
        ...
        if (sz == 0b10) {   
            ...
            if (is_unsgn_offset(bits)) {
                ...
                auto src = state.get_vm_with_offset_32(rn + simm9);
                str_pre_post_idx(src, rn, rt, simm9);
            } else 
            {
    ...
    bool LdrImm::is_pre_post_idx(uint32_t bits) {
        return ((bits & get_mask_from_bits(21, 29)) >> 21) == 0b111000010;
    }
